...
def _guest_init_files() -> Dict[str, str]:
    ...
    **The boot reports whether the UI is actually running.** CI cannot look at a
    ...
    files["etc/init.d/kbb-kiosk"] = (
        ...
        '    # No GPU exists in this machine, so the whole stack must render on the\n'
        '    # CPU. Two layers need telling separately:\n'
        '    #\n'
        '    # cage/wlroots: WLR_RENDERER=pixman is a pure-software renderer that\n'
        '    # never touches EGL/GL. The GL path fails "Unable to create the wlroots\n'
        '    # renderer" because a current Mesa REFUSES LIBGL_ALWAYS_SOFTWARE once\n'
        '    # EGL has opened the virtio-gpu device ("Not allowed to force software\n'
        '    # rendering when API explicitly selects a hardware device"). pixman\n'
        '    # sidesteps that fight entirely.\n'
        '    export WLR_RENDERER=pixman\n'
        '    export WLR_NO_HARDWARE_CURSORS=1\n'
        '    # WebKitGTK: its DMA-BUF path assumes a working GPU and, with virtio-vga\n'
        '    # and no virgl, initialises, falls back and produces no frames.\n'
        '    # Disabling DMA-BUF drops it to a CPU raster path. LIBGL_ALWAYS_SOFTWARE\n'
        '    # is deliberately NOT set -- it is what the newer Mesa rejects.\n'
