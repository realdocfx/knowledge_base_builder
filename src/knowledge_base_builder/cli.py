...
def _guest_init_files() -> Dict[str, str]:
    ...
    **The boot reports whether the UI is actually running.** CI cannot look at a
    ...
    files["etc/init.d/kbb-kiosk"] = (
        ...
        '    # No GPU exists in this machine, so stop the graphics stack looking\n'
        '    # for one. WebKitGTK renders through a DMA-BUF path that assumes a\n'
        '    # working GPU; with virtio-vga and no virgl it initialises, falls back\n'
        '    # to kms_swrast, and then produces no frames. The process stays alive\n'
        '    # and the screen never updates -- a failure no liveness check can see.\n'
