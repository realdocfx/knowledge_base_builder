...
def _write_sandbox_launchers(root: Path, port: int = 8080) -> None:
    ...
    **Two invariants the operator is entitled to.**
    ...
    win_lines = [
        ...
        'SET "CACHE=%LOCALAPPDATA%\\KBB\\sandbox"',
        'IF NOT EXIST "%QEMU%" (',
        ...
        "    exit /b 1",
        ")",
        ":: Cache the sandbox runtime off the stick BEFORE the volume goes away.",
        ":: Raw reads of a mounted volume block, so D: must be dismounted -- and",
        ":: that makes these three files unreachable at the same instant. They are",
        ":: re-derivable from the stick, so this is a cache, not a host dependency.",
        ":: robocopy exits 0-7 for success; only >=8 is a real failure.",
        'IF NOT EXIST "%CACHE%" mkdir "%CACHE%" 2>nul',
        'robocopy "%USB%." "%CACHE%" vmlinuz-kbb initramfs-kbb kbb_guest.img /XO /NJH /NJS /NP /NDL >nul',
        "IF ERRORLEVEL 8 (",
        "    echo [!] Could not cache the sandbox runtime to %CACHE%.",
        "    timeout /t 20 >nul",
        "    exit /b 1",
        ")",
        ":: Resolve the physical disk backing this drive letter. Never hardcoded:",
        ":: a fixed number is correct here and someone else's disk elsewhere.",
        ...
        "    echo [!] Refusing to guess: the wrong disk would be handed to the guest.",
        "    timeout /t 20 >nul",
        "    exit /b 1",
        ")",
        'SET "RAW=\\\\.\\PhysicalDrive%DISKNUM%"',
        ":: Capture the volume identity so the drive letter can be restored, then",
        ":: dismount. While the sandbox runs, %DRV% is gone from Explorer -- that is",
        ":: the price of exclusive raw access, and it is the only way the guest can",
        ":: read the disk without its virtio probe blocking forever.",
        "FOR /F \"tokens=*\" %%v IN ('mountvol %DRV% /L') DO SET \"VOLID=%%v\"",
        'IF "%VOLID%"=="" (',
        "    echo [!] Could not identify the volume behind %DRV%.",
        "    echo [!] Refusing to dismount something that cannot be restored.",
        "    timeout /t 20 >nul",
        "    exit /b 1",
        ")",
        "echo [KBB] Archive device: %RAW% (read-only)",
        "echo [KBB] %DRV% is unmounted while the sandbox runs; restored on exit.",
        "echo [KBB] If this window is closed before the sandbox exits, restore",
        "echo [KBB] the drive with:  Add-PartitionAccessPath -DiskNumber %DISKNUM% -PartitionNumber 1 -AccessPath %DRV%\\",
        ":: Remove the drive letter, NOT `mountvol /P`. /P additionally marks the",
        ":: volume NOT MOUNTABLE, which survives unplugging: a stick detached that",
        ":: way does not come back when replugged, and the operator is left with a",
        ":: drive that has vanished. Removing the access path releases the host's",
        ":: hold just the same and is undone by re-adding it -- or by Windows",
        ":: itself on the next insertion.",
        "powershell -NoProfile -Command \"Remove-PartitionAccessPath -DiskNumber %DISKNUM% -PartitionNumber 1 -AccessPath '%DRV%\' -ErrorAction Stop\"",
        "IF ERRORLEVEL 1 (",
        "    echo [!] Could not dismount %DRV% -- something on the host holds it open.",
        "    echo [!] Close Explorer windows on %DRV% and any KBB portal, then retry.",
        "    echo [!] Continuing would hand the guest a locked device and hang its",
        "    echo [!] disk probe with no error, which is what this check prevents.",
        "    powershell -NoProfile -Command \"Add-PartitionAccessPath -DiskNumber %DISKNUM% -PartitionNumber 1 -AccessPath '%DRV%\'\" >nul 2>&1",
        "    timeout /t 25 >nul",
        "    exit /b 1",
        ")",
        ...
        ":: Flight recorder: guest console and QEMU's own stderr both to %TEMP%.",
        ":: Never onto the stick -- the guest is reading that disk raw.",
        '"%QEMU%" -L "%FW%" -nodefaults -M q35 -m 3072 -smp 2 ^',
        '    -kernel "%CACHE%\\vmlinuz-kbb" -initrd "%CACHE%\\initramfs-kbb" ^',
        f'    -append "{cmdline}" ^',
        '    -drive file="%CACHE%\\kbb_guest.img",format=raw,if=virtio,snapshot=on ^',
        "    -drive file=%RAW%,format=raw,if=virtio,readonly=on ^",
        "    -device virtio-vga ^",
        "    -device virtio-keyboard-pci -device virtio-tablet-pci ^",
        '    -serial file:"%TEMP%\\kbb_sandbox.log" ^',
        '    -full-screen -display sdl,grab-mod=rctrl 2>"%TEMP%\\kbb_qemu.log"',
        ":: Restore the drive letter whatever happened above.",
        "powershell -NoProfile -Command \"Add-PartitionAccessPath -DiskNumber %DISKNUM% -PartitionNumber 1 -AccessPath '%DRV%\'\"",
        "echo [KBB] %DRV% restored.",
    ]
...
def _guest_init_files() -> Dict[str, str]:
    ...
    **The boot reports whether the UI is actually running.** CI cannot look at a
    ...
    files["etc/init.d/kbb-kiosk"] = (
        ...
        '    # A panic inside a dependency reports only a crate line number, which\n'
        '    # is not enough to act on. The backtrace costs nothing here and turns\n'
        '    # "manager.rs:526" into a frame that names the call.\n'
        '    export RUST_BACKTRACE=1\n'
        '    # The portal logs through a redirection, so Python block-buffers its\n'
        '    # output: diagnostics can still be sitting in a buffer when the guest\n'
        '    # stops, which made a healthy run look like a stalled startup.\n'
        ...
        '            # Retry rather than sample once. The portal takes longer to\n'
        '            # come up from USB than from a local disk, and a single probe\n'
        '            # at a fixed instant reported a healthy portal as FAIL on the\n'
        '            # real stick while the identical build passed in CI.\n'
        '            portal_ok=0\n'
        '            probes=0\n'
        '            while [ "$probes" -lt 60 ]; do\n'
        '                wget -q -O /dev/null "http://127.0.0.1:$KBB_PORT/" 2>/dev/null \\\n'
        '                    && portal_ok=1 && break\n'
        '                probes=$((probes+1))\n'
        '                sleep 2\n'
        '            done\n'
        '            if [ "$portal_ok" = "1" ]; then\n'
        '                kbb_log "KBB-PORTAL-OK served / after $probes retries"\n'
        '            else\n'
