...
def _guest_init_files() -> Dict[str, str]:
    ...
    **The boot reports whether the UI is actually running.** CI cannot look at a
    ...
    files["etc/init.d/kbb-kiosk"] = (
        ...
        '\n'
        '    if mountpoint -q "$KBB_RO"; then\n'
        '        kbb_log "archive device mounted read-only at $KBB_RO"\n'
        '        # The portal writes its state directory INTO the bucket, and the\n'
        '        # archive is read-only by design -- that is the whole safety\n'
        '        # argument for handing a sandbox a physical disk. Without an\n'
        '        # overlay the portal dies during startup with\n'
        '        #   OSError: [Errno 30] Read-only file system: .../.kb_state\n'
        '        # The overlay resolves it without relaxing the mount: the archive\n'
        '        # is the lower layer and a tmpfs is the upper, so writes land in\n'
        '        # RAM and are gone at power-off -- the same amnesic property that\n'
        '        # snapshot=on gives the guest image.\n'
        '        modprobe overlay 2>/dev/null || true\n'
        '        mkdir -p /tmp/kbb-upper /tmp/kbb-work\n'
        '        if mount -t overlay overlay \\\n'
        '            -o lowerdir="$KBB_RO",upperdir=/tmp/kbb-upper,workdir=/tmp/kbb-work \\\n'
        '            "$KBB_DATA" 2>/tmp/kbb-overlay.err; then\n'
        '            kbb_log "archive mounted at $KBB_DATA (overlay: ro archive + tmpfs)"\n'
        '        else\n'
        '            # Degrade rather than fail, and say exactly what was lost: a\n'
        '            # read-only bucket still serves anything that needs no writes.\n'
        '            kbb_log "WARNING overlay refused: $(cat /tmp/kbb-overlay.err 2>/dev/null)"\n'
        '            dmesg | grep -i overlay | tail -3 | while read -r l; do kbb_log "dmesg: $l"; done\n'
        '            kbb_log "WARNING the portal cannot write state and may not start"\n'
