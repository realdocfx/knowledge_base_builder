...
- Presentation layer now requires the native `kiwix-serve` binary. The pure-Python `libzim` fallback has been removed from `presentation.py` and `web.py` because it cannot expose the ServiceWorker/REST APIs that ZIM Wikipedia JavaScript needs.
- FastAPI dashboard embeds `kiwix-serve` directly via an `<iframe>` instead of proxying ZIM traffic through `/wiki/`.
- `wiki_orchestrator` defaults changed to `full_flavour="maxi"` and `allow_mini=False` so the 1:1 Wikipedia clone is preserved (mini/nopic variants strip media and interactive assets).
...
[0.4.2]: https://github.com/realdocfx/knowledge_base_builder/releases/tag/v0.4.2
